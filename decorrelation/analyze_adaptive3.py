...
from decorrelation.adaptive_corpus3 import CORPUS
...
def main() -> None:
    ...
    print(f"\nby mutation family, pooled over all {len(P)} evaluated agents:")
    byr = collections.defaultdict(list)
    for k, v in P.items():
        for i, pid in enumerate(pids):
            byr[byround[pid]].append(v[i])
    for r in sorted(byr, key=lambda k: -np.mean(byr[k])):
        arr = np.array(byr[r])
        n = sum(1 for p in pids if byround[p] == r)
        print(f"  {str(r):<20}({n:2d} payloads): approval {arr.mean()*100:5.1f}%")
    ...
    p = os.path.join(HERE, "adaptive3_analysis.json")
